...
import pet.pupgrammer.b4tea4t.LeveledUserCountCache
import pet.pupgrammer.b4tea4t.UserLevelCache
import pet.pupgrammer.b4tea4t.config.Config
import pet.pupgrammer.b4tea4t.database.UserLevelData
import pet.pupgrammer.b4tea4t.database.UserLevelRepository
...
class LevelService(
    ...
    fun calculateXpToReachLevel(level: Int): Float {
        if (level == 0) return 0f // Ensure level 0 starts with 0 XP
    ...
    fun calculateLevelFromXp(xp: Float): Int {
        ...
        // Ensure the discriminant (b^2 - 4ac) is non-negative
        val discriminant = b.pow(2) - 4 * a * c
        if (discriminant < 0) return 0 // Return level 0 if the discriminant is negative

        return ((-b + sqrt(discriminant)) / (2 * a)).toInt().coerceAtLeast(0) // Ensure level is non-negative
    }

    /**
     * Calculates the amount of experience required to reach the next level from the current level.
     *
     * @param level The current level.
     * @return The amount of experience required to reach the next level.
     */
    fun calculateXpToNextLevel(level: Int): Float {
        val totalXpForCurrentLevel = calculateXpToReachLevel(level)
        val totalXpForNextLevel = calculateXpToReachLevel(level + 1)
    ...
    suspend fun getUserLevel(guildId: Snowflake, memberId: Snowflake): UserLevel {
        ...
        // If the user level is null, instantiate a new UserLevel with 0 XP
        if (level == null) {
            level = UserLevel(guildId, memberId, 0f)
        }
    ...
    suspend fun getAllUserLevelsNoCache(guildId: Snowflake): List<UserLevel> {
        return userLevelRepository.findAllByGuildId(guildId.asLong())
            .map(::UserLevel)
            .collectList()
            .awaitSingle()
    }
    ...
    suspend fun removeInactiveMemberLevel(guildId: Snowflake, userId: Snowflake) {
        userLevelRepository.deleteByGuildIdAndMemberId(guildId.asLong(), userId.asLong()).awaitSingleOrNull()

        userLevelCache.evict(guildId, userId)
        leveledUserCountCache.evict(key = guildId)
    }
    ...
    suspend fun handleQualifyingMessage(message: Message) {
        ...
        upsertUserLevel(userLevel.copy(xp = max(0f, userLevel.xp + xpGained)))
    }
}